...
renderCUDA(
	...
	const uint2* __restrict__ ranges_c,
	const uint32_t* __restrict__ point_list_c,
	int nrend,
	...
	float* __restrict__ out_color)
{
	...
	if(ranges_c != nullptr){
		...
		uint2 range_c = ranges_c[tile_id];
	...
	float* __restrict__ out_color)
{
	...
	uint32_t tile_id = block.group_index().y * horizontal_blocks + block.group_index().x;
	uint2 range;
	range.x = ranges[2*tile_id];
	range.y = ranges[2*tile_id+1];
...
void FORWARD::render(
	...
	const uint2* ranges,
	const uint2* ranges_c,
	int nrend,
	const uint32_t* point_list_c,
...
void FORWARD::renderPre(
	...
	float* out_color)
{
	renderCUDAPre<NUM_CHAFFELS> << <grid, block >> > (
		ranges,
		point_list,
		per_tile_bucket_offset, bucket_to_tile,
		sampled_T, sampled_ar,
